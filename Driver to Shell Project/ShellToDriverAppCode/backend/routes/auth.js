...
router.post('/login', (req, res, next) => {
   ...
   db.getDb()
      ...
      .collection('users').findOne({ email: email })
         .then(userDoc => {
            // verify password against hashedPW
            return bcrypt.compare(pw, userDoc.password);
         })
            .then(result => {
               // console.log(result);    // This will not throw any error but will log true or false.
               if (!result) {
                  throw Error();
               }
               const token = createToken();
               res.status(200).json({ message: 'Authentication succeeded.', token: token});
